...
#     1. admin/utils/theme.ts == white-label/utils/theme.ts;
...
#        white-label e no do admin (senão cai no fallback em silêncio).
#
...
if [[ "${1:-}" == "--check" ]]; then
  ...
  if cmp -s "$SRC" "$DST"; then
    echo "theme.ts em sincronia (white-label = admin)"
  else
    echo "admin/utils/theme.ts diverge de white-label/utils/theme.ts — rode scripts/sync-theme.sh" >&2
    status=1
  fi

  SRC="$SRC" API_THEME="$API_THEME" WL_CSS="$WL_CSS" ADMIN_CSS="$ADMIN_CSS" node - <<'NODE' || status=1
const fs = require('fs')
...
for (const [label, file] of [['white-label', process.env.WL_CSS], ['admin', process.env.ADMIN_CSS]]) {
...
if (!failed) console.log(`fontes importadas no white-label e no admin (${families.size} famílias)`)
